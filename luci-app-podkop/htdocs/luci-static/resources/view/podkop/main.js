...
((Podkop2) => {
  ...
  ((AvailableMethods2) => {
    ...
    (AvailableMethods =
      Podkop2.AvailableMethods || (Podkop2.AvailableMethods = {})),
  );
...
var PodkopShellMethods = {
  checkDNSAvailable: async () =>
    callBaseMethod(Podkop.AvailableMethods.CHECK_DNS_AVAILABLE),
  checkFakeIP: async () => callBaseMethod(Podkop.AvailableMethods.CHECK_FAKEIP),
  checkNftRules: async () =>
    callBaseMethod(Podkop.AvailableMethods.CHECK_NFT_RULES),
  getStatus: async () => callBaseMethod(Podkop.AvailableMethods.GET_STATUS),
  checkSingBox: async () =>
    callBaseMethod(Podkop.AvailableMethods.CHECK_SING_BOX),
  getSingBoxStatus: async () =>
    callBaseMethod(Podkop.AvailableMethods.GET_SING_BOX_STATUS),
  getClashApiProxies: async () =>
    callBaseMethod(Podkop.AvailableMethods.CLASH_API, [
      Podkop.AvailableClashAPIMethods.GET_PROXIES,
    ]),
  getClashApiProxyLatency: async (tag) =>
    callBaseMethod(Podkop.AvailableMethods.CLASH_API, [
  ...
  restart: async () =>
    callBaseMethod(Podkop.AvailableMethods.RESTART, [], "/etc/init.d/podkop"),
  start: async () =>
    callBaseMethod(Podkop.AvailableMethods.START, [], "/etc/init.d/podkop"),
  stop: async () =>
    callBaseMethod(Podkop.AvailableMethods.STOP, [], "/etc/init.d/podkop"),
  enable: async () =>
    callBaseMethod(Podkop.AvailableMethods.ENABLE, [], "/etc/init.d/podkop"),
  disable: async () =>
    callBaseMethod(Podkop.AvailableMethods.DISABLE, [], "/etc/init.d/podkop"),
  ...
  getSystemInfo: async () =>
    callBaseMethod(Podkop.AvailableMethods.GET_SYSTEM_INFO),
};
...
async function runDnsCheck() {
  ...
  updateCheckStore({
    ...
    state: getStatus(),
    items: [
      ...insertIf(data.dns_type === "doh" || data.dns_type === "dot", [
        {
...
function renderRunAction({ loading, click }) {
...
async function handleViewLogs() {
  ...
  try {
    ...
    if (viewLogs.success) {
      ui.showModal(_("View logs"), renderModal(viewLogs.data, "view_logs"));
    }
  } catch (e) {
