...
## Режим без FakeIP (опция «Don't use FAKE IP»)

В Additional Settings появилась галочка **«Don't use FAKE IP»**. Когда она включена, Podkop отключает FakeIP-DNS и переходит на маршрутизацию по SNI/Host (sniff-based routing). DNS отвечает реальными IP, диапазон `198.18.0.0/15` нигде не используется.

Когда это нужно:

- На клиенте установлен корпоративный VPN-клиент, который перехватывает DNS на устройстве (например, FortiClient на macOS), и его нельзя поставить на сам роутер. В этой ситуации FakeIP-схема ломается, потому что VPN-клиент не отдаёт приложениям IP-адреса из диапазона FakeIP. Подробности: [Корпоративный VPN](https://podkop.net/docs/workvpn/).

Важно перед включением:

- В этом режиме через sing-box идёт **весь** TCP/UDP-трафик с интерфейсов, перечисленных в `source_network_interfaces`. Убедитесь, что в списке только доверенные LAN-bridge (`br-lan`, гостевые сети), без WAN.

Ограничения и побочные эффекты:

- Sniff не достаёт SNI у доменов, использующих ECH (Encrypted Client Hello) — такие домены не сматчатся.
- IPv6-трафик в этом режиме маршрутизируется напрямую (не через sing-box). Это сделано осознанно: tproxy слушает только на IPv4, а маршрутизация v6 через v4-only listener привела бы к молчаливому отбрасыванию пакетов.
- Через sing-box проходит весь IPv4 TCP/UDP-трафик с настроенных интерфейсов (а не только пакеты с FakeIP-адресами), поэтому нагрузка на CPU роутера выше. На современных моделях (MT7981/Filogic 880, MT7986) это незаметно.
- Диагностический чек «FakeIP» показывает плашку «FakeIP отключён» — это ожидаемое поведение, не ошибка.

После переключения опции необходимо нажать «Save & Apply» и перезапустить Podkop.
